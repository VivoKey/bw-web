...
export class ConsumeOIDCService {
    ...
    async getBearerToken(token: string) {
        let headers: HttpHeaders = new HttpHeaders('Authorization: Basic MTA3MzI4Njg1ODI6M2MwOTAyNjQwOGRhODZkZTJmMTI0NTAyNGQ4YTFhMzE1MDIzNGE3ZDIzNjA1NDExNWQ5OGJlOTc=');
    ...
    async getUserInfo(token: any) {
        let headers2: HttpHeaders = new HttpHeaders({ 'Authorization': "Bearer " + token.access_token })
        try {
